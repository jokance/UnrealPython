# Unreal Python：面向 Unreal Engine 5 的运行时 Python 集成
...
想加快游戏开发速度，可以试试下面这些插件：

- [WidgetSemanticBridge](https://www.fab.com/listings/92270793-0b09-406a-81b9-d6f9f307044f): 使用 AI 快速生成和迭代 UMG，并输出纯文本控件描述 DSL 文件。结合 UnrealPython 与 DSL，可以将全部 UI 逻辑交给 AI 开发。
- [MaterialSemanticBridge](https://www.fab.com/listings/cb2fcfbe-a4db-4dee-a9cf-8bbe62823418): 使用 AI 快速生成和迭代材质，让 AI 成为你的专属 TA。
...
各平台 runtime 构建、裁剪、打包和验证命令见 [Docs/PythonRuntimeBuild.zh.md](Docs/PythonRuntimeBuild.zh.md)。
...
运行时生成 `uclass`、`ustruct`、`uenum`、`uproperty`、`ufunction` 的使用说明和当前支持范围见 [Docs/GeneratedType.zh.md](Docs/GeneratedType.zh.md)。
...
这些 decorator 对应的 Python definition type 已启用 CPython cycle GC 支持。`ue.ufunction()` 当前支持关键字参数形式（如 `Params=[...]`, `Ret=...`）以及生成器支持的 positional decorator 参数路径；具体参数和限制以 [Docs/GeneratedType.zh.md](Docs/GeneratedType.zh.md) 为准。
...
- [UnrealPython 快速入门](Docs/TUTORIAL_UnrealPython_Beginner.zh.md)：从 `ue_site.py`、生命周期回调、对象访问、定时器和清理习惯开始上手。
- [Python Generated Type 使用说明](Docs/GeneratedType.zh.md)：说明 `ue.uclass()`、`ue.ustruct()`、`ue.uenum()`、`ue.uproperty()` 和 `ue.ufunction()` 的用法与限制。
- [GenerationSettings.json 配置说明](Docs/GenerationSettings.zh.md)：说明静态 wrapper 生成范围、成员过滤、`ForceReflection*`、`IsInline`、项目模块输出和验证流程。
- [Python 3.14 Runtime 构建与打包流程](Docs/PythonRuntimeBuild.zh.md)：记录插件自带 Python 运行时的来源、平台目录、构建和打包验证方式。
- [UnrealPython 与 UE PythonScriptPlugin 源码差异](Docs/PythonScriptPluginDifferences.zh.md)：对比本插件和 UE 内置 `PythonScriptPlugin` 的定位、模块结构和运行时差异。
