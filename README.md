...
- [UnrealPython 插件使用说明文档](#unrealpython-插件使用说明文档)
  ...
  - [使用方法](#使用方法)
    ...
    - [Generated Type 支持矩阵](#generated-type-支持矩阵)
    - [Reload / 热更新](#reload--热更新)
...
3. **编写 Python 脚本**:
   ...
   `after_shutdown()` 是可选接口，会在 `shutdown()` 返回之后、Python GameInstance 对象释放之前调用。
...
推荐用 `ue.ReloadModule(module_or_name)` 重新加载项目 Python 模块。它接受模块对象或模块名字符串，内部调用 `importlib.reload()`，并在 reload 后自动执行 generated type reinstancing flush。

```python
import my_gameplay_module
import ue

my_gameplay_module = ue.ReloadModule(my_gameplay_module)
# 或：
my_gameplay_module = ue.ReloadModule("my_gameplay_module")
```

如果你手动用 `importlib.reload()` 或动态生成多个同名 `uclass` / `ustruct`，可以调用 `ue.FlushGeneratedTypeReinstancing()` 强制处理待重实例化类型。
