...
`AddPythonOwned()` 用于让 Python 持有运行时创建的 UObject，避免对象过早被 Unreal GC 清理。生命周期结束后可调用 `RemovePythonOwned()`。Generated Type 的完整规则见 [GeneratedType.zh.md](GeneratedType.zh.md)。
