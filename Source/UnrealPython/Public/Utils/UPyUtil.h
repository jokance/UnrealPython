...
#define UPyCFunctionCast(FUNCPTR) (PyCFunction)(void*)(FUNCPTR)

/** Cast a string literal to a char* (via a void* to avoid a compiler warning) */
#define UPyCStrCast(STR) (char*)(void*)(STR)
...
namespace UPyUtil
{
	...
	/** Attempt to run a generated init overload, clearing errors on failure */
	template <typename SelfType, typename InitFuncType>
	bool TryInit(SelfType* InSelf, PyObject* InArgs, PyObject* InKwds, InitFuncType InitFunc)
	{
		if (InitFunc && InitFunc(InSelf, InArgs, InKwds) == 0)
		{
			return true;
		}
		PyErr_Clear();
	...
	#define UPY_UFUNCTION_STACK(NAME, FUNCTION) \
		UPyUtil::FFunctionStackOnScope NAME(FUNCTION, FMemory_Alloca_Aligned(FMath::Max(1, (FUNCTION)->GetStructureSize()), (FUNCTION)->GetMinAlignment()))
	...
	bool InspectFunctionArgs(PyObject* InFunc, TArray<FString>& OutArgNames, TArray<FUPyObjectPtr>* OutArgDefaults = nullptr);
	 ...
	 */
	UNREALPYTHON_API FString GetFriendlyTypename(PyTypeObject* InPyType);

	/**
	 * Get the friendly typename of the given object that can be used in error reporting.
	 * @note Passing a PyTypeObject returns the name of that object, rather than 'type'.
	 */
	UNREALPYTHON_API FString GetFriendlyTypename(PyObject* InPyObj);
	...
	UNREALPYTHON_API void SetPythonError(PyObject* InException, PyTypeObject* InErrorContext, const TCHAR* InErrorMsg);
	UNREALPYTHON_API void SetPythonError(PyObject* InException, PyObject* InErrorContext, const TCHAR* InErrorMsg);
	UNREALPYTHON_API void SetPythonError(PyObject* InException, const TCHAR* InErrorContext, const TCHAR* InErrorMsg);
